...
const he: GeoContent = {
  ...
  faq: {
    ...
    items: [
      {
        question: 'האם נדרש היתר בנייה להתקנת פרגולה?',
        answer:
          'בהתאם לתקנות התכנון והבנייה בישראל, פרגולות בגודל וגובה מסוימים עשויות להיכלל בפטור מהיתר, אך הדבר תלוי בשטח המבנה, בגובהו, במרחק מגבולות המגרש ובתקנות העירייה הספציפית. אנחנו ממליצים לבדוק את הנחיות הוועדה המקומית באזור מגוריכם לפני התקנה, ובמידת הצורך אנחנו מסייעים בהכנת המסמכים הנדרשים כחלק מהשירות.',
      },
      {
        question: 'כמה עומדת פרגולת אלומיניום בפני רוח וגשם?',
        answer:
          'כל המבנים שלנו מתוכננים ומיוצרים לעמידה בתנאי מזג האוויר בישראל, כולל רוחות עונתיות וגשם חזק. פרגולות חשמליות ביוקלימטיות מגיעות עם מערכת ניקוז נסתרת ואטימות מלאה כשהשלבים סגורים, ולעיתים ניתן להוסיף חיישני רוח וגשם לסגירה אוטומטית במקרה הצורך.',
      },
      {
        question: 'מה ההבדל בין פרגולה ביוקלימטית לפרגולת PVC?',
        answer:
          'פרגולה ביוקלימטית מבוססת על שלבי אלומיניום מתכווננים בזווית של 0° עד 90°, המאפשרים שליטה מדויקת באור, בצל ובאוורור. פרגולת PVC, לעומת זאת, מבוססת על גג יריעה גמיש ועמיד למים ואינה מציעה שליטה בזווית - היא נפתחת או נסגרת כמקשה אחד. הבחירה בין השתיים תלויה בעיקר ברמת השליטה הנדרשת באור ובאוורור לאורך היום.',
      },
      {
        question: 'איזו אחריות מקבלים על המבנה?',
        answer:
          'Vitlion Group מספקת אחריות כתובה של 10 שנים על כל מבנה שאנחנו מייצרים ומתקינים, הכוללת כיסוי על תקינות המבנה והמנגנונים החשמליים בשימוש רגיל.',
      },
      {
        question: 'האם פרגולת אלומיניום דורשת תחזוקה שוטפת?',
        answer:
          'לא. בשונה מפרגולות עץ, אלומיניום אינו דורש צביעה או טיפול תקופתי מפני חלודה או ריקבון. שטיפה קלה במים מדי פעם מספיקה לשמירה על המראה והתפקוד התקין של המבנה לאורך שנים.',
      },
    ],
  },
}
...
const ru: GeoContent = {
  ...
  faq: {
    ...
    items: [
      {
        question: 'Нужно ли разрешение на строительство для установки перголы?',
        answer:
          'В соответствии с нормами планирования и строительства в Израиле, перголы определённого размера и высоты могут подпадать под освобождение от разрешения, однако это зависит от площади конструкции, её высоты, расстояния от границ участка и конкретных требований местного муниципалитета. Мы рекомендуем уточнить правила местного комитета по планированию в вашем районе перед установкой, а при необходимости мы помогаем в подготовке требуемых документов в рамках нашей услуги.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Биоклиматическая пергола основана на регулируемых алюминиевых ламелях с углом наклона от 0° до 90°, что позволяет точно контролировать освещённость, тень и вентиляцию. Пергола из ПВХ, в свою очередь, основана на гибком и водостойком полотняном покрытии и не предполагает регулировки угла — она открывается или закрывается как единое целое. Выбор между этими двумя типами зависит главным образом от того, насколько важен для вас точный контроль над светом и вентиляцией в течение дня.',
      },
      {
        question: 'Какая гарантия предоставляется на конструкцию?',
        answer:
          'Vitlion Group предоставляет письменную гарантию на 10 лет на каждую произведённую и установленную нами конструкцию, включающую покрытие исправности самого сооружения и электрических механизмов при обычном использовании.',
      },
      {
        question: 'Требует ли алюминиевая пергола постоянного обслуживания?',
        answer:
          'Нет. В отличие от деревянных пергол, алюминий не требует окраски или периодической обработки от коррозии или гниения. Лёгкого мытья водой время от времени достаточно для поддержания внешнего вида и исправной работы конструкции на протяжении многих лет.',
      },
    ],
  },
}
